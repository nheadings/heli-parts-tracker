    ...
    const result = await pool.query(
      `UPDATE maintenance_schedules
       SET title = $1, description = $2, interval_hours = $3, interval_days = $4,
           ...
           display_in_flight_view = $9, threshold_warning = $10, logbook_category_id = $11
       WHERE id = $12
       RETURNING *`,
      [title, description, interval_hours, interval_days, category, is_active,
       color, display_order, display_in_flight_view, threshold_warning, logbook_category_id, id]
    );
...
router.get('/helicopters/:helicopterId/dashboard', async (req, res) => {
  ...
  try {
    ...
    // Uses categories directly - no more maintenance_schedules table
    const flightViewMaintenanceResult = await pool.query(
      `SELECT lc.id, lc.name as title, lc.interval_hours, lc.color, lc.display_order, lc.threshold_warning,
              lc.id as logbook_category_id,
              le.hours_at_event as last_completed_hours,
              le.hours_at_event + lc.interval_hours as next_due_hours,
              CASE
                WHEN le.hours_at_event IS NOT NULL
                THEN (le.hours_at_event + lc.interval_hours) - $1
                ELSE lc.interval_hours
              END as hours_remaining
       FROM logbook_categories lc
       LEFT JOIN LATERAL (
         SELECT hours_at_event
       ...
       LEFT JOIN helicopter_category_banners hcb
         ON hcb.category_id = lc.id AND hcb.helicopter_id = $2 AND hcb.is_enabled = true
       WHERE lc.is_active = true
         AND lc.display_in_flight_view = true
         AND lc.interval_hours IS NOT NULL
         AND (
           hcb.id IS NOT NULL
           OR NOT EXISTS (
             SELECT 1 FROM helicopter_category_banners
             WHERE category_id = lc.id
           )
         )
